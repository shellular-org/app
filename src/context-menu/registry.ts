import { getKeybindingsSnapshot } from "lib/keybindings";
import {
...
const KEYBINDING_COMMANDS: Partial<Record<CommandId, DesktopKeyboardCommand>> =
	{
		"edit.undo": "undo",
		"edit.redo": "redo",
		"edit.cut": "cut",
		...
		"editor.definition": "editor.definition",
		"editor.peekDefinition": "editor.peekDefinition",
		"editor.references": "editor.references",
		"editor.renameSymbol": "editor.renameSymbol",
		"editor.formatDocument": "editor.formatDocument",
		"tab.close": "close-tab",
	};
...
function resolveCommand(
	...
	const keybindingCommand = KEYBINDING_COMMANDS[id];
	const keybinding = keybindingCommand
		? shortcutForCommand(
				keybindingCommand,
				platform,
				getKeybindingsSnapshot().overrides,
			)
		: undefined;
	const shortcutValue =
		keybinding?.strokes.length === 1
			? {
					key: keybinding.strokes[0].key,
					modifiers: keybinding.strokes[0].modifiers,
				}
			: definition.shortcuts?.[platform];
	return [
		{
			type: "command",
			command: id,
			...
			shortcutLabel: keybinding
				? formatShortcutBinding(keybinding, platform)
				: shortcutValue
...
export function formatShortcut(
	...
	const modifiers = (["ctrl", "shift", "alt", "meta"] as const).filter(
...
function browserPlatform(): "mac" | "windows" | "linux" {
	return detectDesktopShortcutPlatform();
}
