...
describe("context-menu registry", () => {
	...
	it("resolves labels, state, shortcuts, and checked radio items at invocation time", () => {
		const dispose = registerCommandDefinition({
			...
			shortcuts: {
				...
				windows: { key: "k", modifiers: ["ctrl"] },
				linux: { key: "k", modifiers: ["ctrl"] },
			},
		});
		try {
	...
	it("formats platform shortcuts", () => {
		...
		expect(
			formatShortcut({ key: "F12", modifiers: ["shift"] }, "windows"),
		).toBe("Shift+F12");
		expect(
			formatShortcut({ key: "f", modifiers: ["alt", "shift"] }, "mac"),
		).toBe("⇧⌥F");
	});

	it("keeps Windows and Linux VS Code editor bindings distinct", () => {
		const target = {
			handlers: {
				"edit.redo": { run: vi.fn() },
				"editor.formatDocument": { run: vi.fn() },
				"editor.peekDefinition": { run: vi.fn() },
			},
		};
		const labels = (platform: "windows" | "linux") =>
			resolveCommandGroups(
				[["edit.redo"], ["editor.formatDocument"], ["editor.peekDefinition"]],
				target,
				platform,
			)
				.filter((item) => item.type === "command")
				.map((item) => item.shortcutLabel);

		expect(labels("windows")).toEqual(["Ctrl+Y", "Shift+Alt+F", "Alt+F12"]);
		expect(labels("linux")).toEqual([
			"Ctrl+Y",
			"Ctrl+Shift+I",
			"Ctrl+Shift+F10",
		]);
