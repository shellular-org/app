...
export async function createProjectChild(
...
): Promise<CreatedProjectChild | null> {
	const label = type === "file" ? "file" : "folder";
	const name =
		requestedName ??
		(await dialog.textInput(
	...
	try {
		...
		const mutation: CreatedProjectChild = {
			type: "add",
			path,
			entryType: type,
		};
...
export type CreatedProjectChild = Extract<
	AbsoluteProjectTreeMutation,
	{ type: "add" }
>;
