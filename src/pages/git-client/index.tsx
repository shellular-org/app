...
export default function GitClientPage({ projectPath, projectName }: Props) {
	const { connectionStatus, runGitOperation, getGitLog } = useShellular();
...
function ChangesTab({
	...
	const hasUnstaged = status.files.some((f) => f.unstaged) ?? false;
	...
	const handleUnstageAll = () => {
		...
		if (filesToUnstage.length > 0) {
			run("unstage", { files: filesToUnstage });
		}
	};
				...
				<div className="git-change-toolbar">
					...
					<div className="git-toolbar-actions">
						...
						{hasUnstaged && (
							<button
								type="button"
								className="git-toolbar-btn git-btn-success"
						...
						{hasStaged && (
							<button
								type="button"
								className="git-toolbar-btn git-btn-danger"
