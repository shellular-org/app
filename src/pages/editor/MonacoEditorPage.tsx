	...
	useEffect(() => {
		...
		const unregisterCommands = registerWorkbenchCommandHandlers(id, {
			...
			"editor.peekDefinition": {
				...
				enabled: () =>
					Boolean(
						editor()?.getAction("editor.action.peekDefinition")?.isSupported(),
					),
			},
			"editor.references": {
				run: () => editor()?.getAction("editor.action.goToReferences")?.run(),
				enabled: () =>
					Boolean(
						editor()?.getAction("editor.action.goToReferences")?.isSupported(),
					),
			},
			"editor.renameSymbol": {
				...
				enabled: () =>
					Boolean(
						editable() &&
							editor()?.getAction("editor.action.rename")?.isSupported(),
					),
			},
